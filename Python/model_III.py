...
HA2RL   = 0.2  # half-saturation const

zJ1A1   = 0.1  # juvenile-to-adult mass ratio in consumer 1
zJ2A2   = 0.1  # juvenile-to-adult mass ratio in consumer
...
IJ1RSmax = (19 * (B_J1**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_J1
IA1RSmax = (19 * (B_A1**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_A1
IA1RLmax = (19 * (B_A1**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_A1
IJ2RSmax = (19 * (B_J2**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_J2
IJ2RLmax = (19 * (B_J2**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_J2
IA2RLmax = (19 * (B_A2**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_A2

# Consumer metabolic rate:
mJ1      = (850000000 * (B_J1**0.7) * np.exp( -0.56/(0.00008617*T) )) / B_J1
mA1      = (850000000 * (B_A1**0.7) * np.exp( -0.56/(0.00008617*T) )) / B_A1
mJ2      = (850000000 * (B_J2**0.7) * np.exp( -0.56/(0.00008617*T) )) / B_J2
mA2      = (850000000 * (B_A2**0.7) * np.exp( -0.56/(0.00008617*T) )) / B_A2
"""
...
# Consumer maximum ingestion rate:
IJ1RSmax = (19 * (B_J1**(0.7)) * np.exp(-((T-(273.15+24))**2)/(2*(8**2)))) / B_J1
IA1RSmax = (19 * (B_A1**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_A1
IA1RLmax = (19 * (B_A1**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_A1
IJ2RSmax = (19 * (B_J2**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_J2
IJ2RLmax = (19 * (B_J2**(0.7)) * np.exp(-((T-(273.15+20))**2)/(2*(8**2)))) / B_J2
IA2RLmax = (19 * (B_A2**(0.7)) * np.exp(-((T-(273.15+16))**2)/(2*(8**2)))) / B_A2
...
# Consumer metabolic rate:
mJ1      = (850000000 * (B_J1**(0.7 + 0.0005*T)) * np.exp( -0.56/(0.00008617*T) )) / B_J1
mA1      = (850000000 * (B_A1**(0.7 + 0.0005*T)) * np.exp( -0.56/(0.00008617*T) )) / B_A1
mJ2      = (850000000 * (B_J2**(0.7 + 0.0005*T)) * np.exp( -0.56/(0.00008617*T) )) / B_J2
mA2      = (850000000 * (B_A2**(0.7 + 0.0005*T)) * np.exp( -0.56/(0.00008617*T) )) / B_A2
"""
...
def model(X,t):
    ...
    J1 = X[2]   # juvenile 1 biomass density
    A1 = X[3]   # adult 1 biomass density
    J2 = X[4]   # juvenile 2 biomass density
    A2 = X[5]   # adult 2 biomass density
    ...
    return np.array([dRSdt, dRLdt, dJ1dt, dA1dt, dJ2dt, dA2dt])

# Initial densities for RS, RL, J1, A1, J2, A2
...
# Plot results
RS,RL,J1,A1,J2,A2 = np.transpose(X)
...
plt.figure()
plt.plot(t, J1, 'k-', label='J1', linewidth=1.0)
plt.plot(t, A1, 'k-', label='A1', linewidth=2.5)
plt.legend(loc='upper right')
plt.xlabel('Time (day)')
plt.ylabel('Density (mg/L)')
plt.show()
...
plt.figure()
plt.plot(t, J2, 'k-', label='J2', linewidth=1.0)
plt.plot(t, A2, 'k-', label='A2', linewidth=2.5)
plt.legend(loc='upper right')
plt.xlabel('Time (day)')
plt.ylabel('Density (mg/L)')
plt.show()